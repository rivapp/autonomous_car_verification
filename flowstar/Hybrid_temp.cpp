...
//				printf("Only one interseted flowpipe.\n");
				...
				*/
				...
				if(!strncmp(modeName.c_str(), "DNN", strlen("DNN"))){
					  ...
					  There is probably a way around this but this works for now.
					*/
				        Continuous_Reachability_Setting crs;
					crs.setFixedStepsize(0.01);
					crs.setFixedOrder(3);
					crs.setPrecision(1000);
					...
					*/
					...
					ResetMap activation_reset(tmv_activation_reset, is_identity);
					...
					//Finally, after_activation_reset creates a normalized Flowpipe for the f states.
					after_activation_reset = Flowpipe(X0, intZero);

					after_activation_reset.intEvalNormal(all_ranges, crs.step_end_exp_table, crs.cutoff_threshold);
					
					Flowpipe after_linear_reset;

					if(!loadedDNN){
					        std::vector<ResetMap> load_dnn_resets;
						std::vector<std::string> load_dnn_activations;
						//load the DNN
						dnn::load_dnn(load_dnn_resets, load_dnn_activations, realStateVars, dnn::DNN_Filename);

						dnn_resets = load_dnn_resets;
						dnn_activations = load_dnn_activations;
						loadedDNN = true;
					}

					// for(int layer = 0; layer < dnn_resets.size(); layer++){
					//   for(int varInd = 0; varInd < realVarNames.size() - 1; varInd++){
					//         Polynomial poly = dnn_resets[layer].tmvReset.tms[varInd].expansion;
					// 	std::string printing;
					...
					for(int layer = 0; layer < dnn_resets.size(); layer++){
					        ...
					        dnn_resets[layer].reset(after_linear_reset, after_activation_reset, crs);

					        //if no activation function, just set after_activation_reset equal to after_linear_reset
					        if(strncmp(dnn_activations[layer].c_str(), "Sigmoid", strlen("Sigmoid")) &&
						   strncmp(dnn_activations[layer].c_str(), "Tanh", strlen("Tanh")) &&
						   strncmp(dnn_activations[layer].c_str(), "Swish", strlen("Swish")) &&
						   strncmp(dnn_activations[layer].c_str(), "Relu", strlen("Relu"))){

						        after_activation_reset = after_linear_reset;
						        continue;
						}

						after_linear_reset.intEvalNormal(all_ranges, crs.step_end_exp_table, crs.cutoff_threshold);
						...
						for(int varInd = 0; varInd < tmvAggregation.tms.size(); varInd++){
						        ...
						        if(stateVarNames[varInd][0] != 'f') break;
							...
							else if(!strncmp(dnn_activations[layer].c_str(), "Tanh", strlen("Tanh"))){
							        dnn::tanh_reset(new_tm_reset, intC, varInd, numVars);
							}
							
							else if(!strncmp(dnn_activations[layer].c_str(), "Swish", strlen("Swish"))){
							        dnn::swish_reset(new_tm_reset, intC, varInd, numVars);
							}
							else if(!strncmp(dnn_activations[layer].c_str(), "Relu", strlen("Relu"))){
							        dnn::relu_reset(new_tm_reset, intC, varInd, numVars);
							}
							
														
							activation_reset.tmvReset.tms[varInd] = new_tm_reset;
						}

						activation_reset.reset(after_activation_reset, after_linear_reset, crs);
					}
				...
				//Code added by Rado for DNN resets
				/*
